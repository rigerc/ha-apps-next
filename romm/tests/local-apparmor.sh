#!/bin/bash
# Exercise the RomM smoke suite under its shipped AppArmor policy.
...
readonly PROFILE_NAME="romm-local-test-${UID}-$$"
TEST_ROOT="$(mktemp -d -t romm-apparmor.XXXXXX)"
readonly TEST_ROOT
readonly PROFILE_FILE="${TEST_ROOT}/apparmor.txt"
readonly DEFAULT_IMAGE="local/ha-addon-romm:apparmor-test"

PROFILE_LOADED=false
ROOT_COMMAND=()

log() {
  printf '[romm-apparmor] %s\n' "$*"
}

fail() {
  printf '[romm-apparmor] ERROR: %s\n' "$*" >&2
  exit 1
}
...
build_image_if_needed() {
  ...
  version="$(awk '$1 == "version:" {print $2; exit}' \
    "${APP_DIR}/config.yaml")"
  [[ -n "${version}" ]] || fail "Unable to read the RomM app version"
  log "building ${image} for RomM ${version}"
  docker build \
...
load_profile() {
  sed "s/^profile romm /profile ${PROFILE_NAME} /" \
    "${PROFILE_SOURCE}" >"${PROFILE_FILE}"
  grep -q "^profile ${PROFILE_NAME} " "${PROFILE_FILE}" \
    || fail "Unable to create an isolated AppArmor profile"
...
read_denials() {
  ...
  if command -v journalctl >/dev/null 2>&1; then
    ...
    log "journalctl is unavailable; inspect kernel logs for denials"
  fi
}
...
run_confined_suite() {
  ...
  started_at="$(date +%s)"
  log "running the RomM suite under AppArmor enforcement"
  ...
  ((status == 0)) || fail "Confined suite failed with status ${status}"
}
...
main() {
  ...
  log "all confined RomM checks passed"
}
